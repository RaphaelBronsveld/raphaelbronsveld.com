...
Actions that _change_ data. For example, there could be a `CreateTodoCommand` class, or a `UpdateTodoCommand`. Their purpose is explicit: something is going to change.

### Queries

These only _read_ data. Something like `GetTodoById`, or `GetAllTodos`. These never modify state, they return information.
